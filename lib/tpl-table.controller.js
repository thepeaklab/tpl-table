...
export default class TplTableCtrl {
  ...
  setEntriesPerPageCount(entriesPerPageCount, callback) {
    var oldEntriesPerPageCount = vm.opts.entriesPerPageCount;
  ...
  setSearch(search, callback) {
    ...
    if (callback && this.onSearchChange) {
  ...
  handleSearchChange(newVal, oldVal) {
    if ((oldVal === '' || !oldVal) && vm.opts.searchModel !== oldVal) { // Search started
      ...
      if (state) {
        if (state.pageBeforeSearch >= 0) {
          ...
          if (state.entriesPerPageCountBeforeSearch > 0) {
            this.setEntriesPerPageCount(state.entriesPerPageCountBeforeSearch); //TODO: callback
          }
          ...
          if (state) {
            if (state.actualPage >= 0) {
              ...
              if (state.actualSearch) {
                this.setSearch(state.actualSearch);
              }

              if (state.actualEntriesPerPageCount > 0) {
                this.setEntriesPerPageCount(state.actualEntriesPerPageCount); //TODO: callback
              }
